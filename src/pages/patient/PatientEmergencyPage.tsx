...
export default function PatientEmergencyPage() {
  ...
  const handleConfirmEmergency = async () => {
    ...
    try {
      // Always try to get the latest location at send time.
      const liveLocation = await getLocation();

      // Get all linked caregivers
      const caregivers = await getLinkedCaregivers(patient.id);
      if (caregivers.length === 0) {
        throw new Error('No caregivers linked to this patient.');
      }

      const alertLocation = liveLocation ?? location;
      ...
      const alertPromises = caregivers.map((caregiver) =>
        createAlert({
          ...
          alert_status: 'unread',
          location_lat: alertLocation?.lat ?? null,
          location_lng: alertLocation?.lng ?? null,
      ...
      toast({
        ...
        description: `Alert sent to ${caregivers.length} caregiver(s)${alertLocation ? ' with your live location' : ''}`,
      });
